...
if [ -z $1 ]; then ## run
  ...
  check_port $ENV_LOTUS_MINER_PORT
  if [ $port -le 0 ]; then
    ...
    if [ $pid -le 3 ]; then
      ...
      sed -i "s/.*  ListenAddress = .*/  ListenAddress = \"\/ip4\/${localip}\/tcp\/${ENV_LOTUS_MINER_PORT}\/http\"/g"  $LOTUS_MINER_PATH/config.toml
    ...
    # tips
    echo -e "\033[31m $ENV_LOTUS_MINER_PORT is used. \033[0m"
  fi
  echo " "
